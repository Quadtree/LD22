...
public class GameState implements Serializable {
	...
	public void setCamera(float x, float y, float viewWidth, float viewHeight)
	{
		...
		camX = x;
		camY = y;
		if(cameraMat == null) cameraMat = new Matrix4();
	...
	public void render()
	{
		...
		if(cameraMat == null) setCamera(0,0, 16,12);
		...
		for(Actor a : actors)
		{
			if(a instanceof Entity && ((Entity)a).getPosition().sub(camX, camY).len2() > clip) continue;
			a.render(batch);
		}
	...
	public void gameStart()
	{
		...
		for(Actor a : actors)
		{
			if(a instanceof Player) conObj = (ControlledObject) a;
			a.gameStart();
		}
	}
	...
	public void update()
	{
		if(addActorQueue == null) addActorQueue = new ArrayList<Actor>();
		...
		for(int i=0;i<actors.size();++i)
		{
			...
			if(actors.get(i) instanceof Player) player = (Player) actors.get(i);
		}
